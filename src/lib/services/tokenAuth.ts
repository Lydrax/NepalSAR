/**
 * Pure TypeScript SHA-256 implementation compatible with Edge Runtime, Node.js, and Browsers.
 * Removes dependency on Node's native 'crypto' module for Cloudflare Pages Edge compatibility.
 */
function sha256Sync(ascii: string): string {
  ...
  for (let i = 0; i < words.length; i += 16) {
    ...
    for (let idx = w.length; idx < 16; idx++) w.push(0);
...
function getRandomIntCrypto(min: number, max: number): number {
  const range = max - min;
...
export function generateVerificationToken(): string {
  ...
  const pin = getRandomIntCrypto(100000, 1000000);
  return pin.toString();
}
...
export function generateNumericCaseNumber(): string {
  ...
  const sequenceNum = getRandomIntCrypto(100000, 1000000).toString();
